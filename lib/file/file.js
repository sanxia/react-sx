...
var SxFile = React.createClass({
    ...
    getAcceptedFileTypes: function(fileTypeCodes){
    ...
    onFileChange: function(e){
        ...
        if(!checkStatus[0]){
            alert("onfile 文件类型不符合要求")
        }else if(!checkStatus[1]){
            alert("onfile 文件大小不符合要求")
        }else{
    ...
    render: function() {
        ...
        return <div className = "sx-file" >
                ...
                <input ref = { (file) => {this.file = file}} type = "file" accept = { accept } multiple className="sx-file-input" onChange = { this.onFileChange } />
                :
                <input ref = { (file) => {this.file = file}} type = "file" accept = { accept } className="sx-file-input" onChange = { this.onFileChange } />
                }
        </div>
    }
})
