...
var SxPagingBar = React.createClass({
    ...
    render: function() {
        ...
        return <div className = "sx-paging" >
                ...
                <SxPagingBeforeAfter isBefore = { true } { ...options } { ...this.props } onClick = {this.onPaging} />
                <SxPagingList { ...options } { ...this.props }  onClick = {this.onPaging} />
                <SxPagingBeforeAfter isBefore = { false } { ...options } { ...this.props } onClick = {this.onPaging} />
...
class SxPagingFirst extends Component {
	render() {
        ...
        let html = ids.map(
            (v,i) => 
	        <span>
                <SxPagingNumber title = { this.props.first_title } text = { this.props.first_text } pagingIndex = { 1 } { ...this.props } />
                <SxPagingNumber title = { this.props.prev_title } text = { this.props.prev_text } pagingIndex = { pagingIndex } { ...this.props } />
            </span>
        )
...
class SxPagingBeforeAfter extends Component {
	render() {
        ...
        var html = ids.map(
            (v,i) => <SxLink title = { title } onLink = { this.props.onClick.bind(this, pagingIndex, this.props.data) } title = { title } >..</SxLink>
        )
...
class SxPagingList extends Component {
    render(){
    	...
    	var pagingList = pageIndexs.map(
            ...
            : 
            <SxPagingNumber key = { i } pagingIndex = { v } {...this.props} />
    	)
...
class SxPagingLast extends Component {
	render() {
        ...
        let html = ids.map(
            (v,i) => 
	        <span key = { i }>
                <SxPagingNumber key = { i } title = { this.props.next_title } text = { this.props.next_text } pagingIndex = { pagingIndex } { ...this.props } />
                <SxPagingNumber key = { i } title = { this.props.last_title } text = { this.props.last_text } pagingIndex = { pagingCount } { ...this.props } />
            </span>
        )
