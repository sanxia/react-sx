...
class SxTextField extends Component{
    ...
    renderHeader = ()=>{
        if(this.props.title){
            ...
            onClick = { ()=>this.onMouseOver() } >
                <span className = "sx-input-title" style = { titleStyle }>{ this.props.title }</span>
                {
    ...
    render = ()=>{
        let inputClassName = this.getClassName()
        let inputBodyClassName = this.getBodyClassName()

        return <div className = { inputClassName }>
            { this.renderHeader() }

            <div
            className = { inputBodyClassName }
