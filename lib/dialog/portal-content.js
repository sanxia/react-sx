    ...
    renderHeader = ()=>{
        if(this.props.isHeader){
            return <div className = "sx-dialog-header">
                <span className = "sx-dialog-header-icon" />
                <div className = "sx-dialog-header-title" dangerouslySetInnerHTML = {{__html: this.state.title}} />
                {
                    this.props.onClose && <span className = "sx-dialog-header-close" title = "关闭" onClick = { ()=>this.onClose() } />
                }
            </div>
        }
